...
def compute_loss(predictions, targets, model):
    ...
    for layer_index, layer_predictions in enumerate(predictions):
        ...
        if num_targets:
            ...
            if ps.size(1) - 5 > 1:
                ...
                # ---------------------shadow class applying------------------------------
                # shadow_class = torch.full_like(tcls[layer_index], 16, device=device) 
                # t[range(num_targets), shadow_class] = 1
                # ------------------------------------------------------------------------

                # ---------------------re-labeling loss------------------------------
                # target_class = torch.full_like(tcls[layer_index], 12, device=device) 
                # t[range(num_targets), target_class] = 1
                # ------------------------------------------------------------------------
    ...
    return loss, to_cpu(torch.cat((lbox, lobj, lcls, loss))), lcls


def build_targets(p, targets, model):
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
    na, nt = 3, targets.shape[0]  # number of anchors, targets #TODO
    # na, nt = 3, targets.shape[1]
