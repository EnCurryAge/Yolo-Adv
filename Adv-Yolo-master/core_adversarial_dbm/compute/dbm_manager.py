...
class DBMManager:
    ...
    def get_dbm_data(self, blocking=True):
        if blocking and self._dbm_data is None:
            ...
            # )
            self._dbm_data = (
                self.classifier.classify_features(self.inverter(self.grid)).cpu().numpy()
            )
            # self._dbm_data = (
            #     self.classifier.classify(self.inverter(self.grid).view(-1, 3, 32, 32)).cpu().numpy()
            # )
            # results = []
            # batch_size = 512
            # for i in range(0, self.grid.size(0), batch_size):
            #     # Slice the batch from the grid
            #     grid_batch = self.grid[i:i + batch_size]  # shape: [batch_size, 2]
                
            #     # Invert to input space
            #     inverted = self.inverter(grid_batch)      # shape: [batch_size, 3072]
                
            #     # Reshape to image shape
            #     images = inverted.view(-1, 3, 32, 32)      # shape: [batch_size, 3, 32, 32]
                
            #     # Classify
            #     # logits = self.classifier(images)          # output: [batch_size, num_classes]
            #     # preds = logits.detach().cpu().numpy()              # convert to numpy

            #     preds = self.classifier.classify(images).cpu().numpy()
                
            #     results.append(preds)

            # # Concatenate all predictions
            # self._dbm_data = np.concatenate(results, axis=0)
    ...
    def get_dbm_data_yolo(self, blocking=True, imgsize = 320):
        if blocking and self._dbm_data is None:

            patch = self.inverter(self.grid).unsqueeze(2).unsqueeze(3)# 把 [n, 1024] 转换为 [n, 1024, 1, 1]

            output_patch = self.classifier.module_list[81][0](patch.to("cuda")) # 把 [n, 1024, 1, 1]转换为 [n, 255, 1, 1]
            yolo_output_patch = self.classifier.module_list[82][0](output_patch,imgsize) #转换为 [n, 3, 85]
            # 和逆矩阵训练保持一致，只取第一个anchor的类别
            _,self._dbm_data = T.max(yolo_output_patch[:,0,5:], dim=1)
            self._dbm_data = self._dbm_data.cpu().numpy()
